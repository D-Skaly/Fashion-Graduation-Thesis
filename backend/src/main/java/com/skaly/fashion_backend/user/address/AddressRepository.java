...
public interface AddressRepository extends JpaRepository<Address, UUID> {
    ...
    List<Address> findByUserIdAndType(UUID userId, Address.AddressType type);

    Optional<Address> findByUserIdAndIsDefaultTrue(UUID userId);

    @Modifying
    @Query("UPDATE Address a SET a.isDefault = false WHERE a.user.id = :userId AND a.type = :type")
    int unsetDefaultAddresses(@Param("userId") UUID userId, @Param("type") Address.AddressType type);
